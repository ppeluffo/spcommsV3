...
class BD_GDA:
    ...
    def __init__(self):
        ...
        self.handle = None
        self.engine = None
        self.url = Config['BDCONFIG']['url_gda_spymovil']
        self.response = False
    ...
    def exec_sql(self, dlgid, sql):
        ...
        try:
            ...
            log(module=__name__, function='exec_sql', level='ERROR', dlgid=dlgid,
                msg='ERROR: SQLQUERY: {0}'.format(sql))
            log(module=__name__, function='exec_sql', level='ERROR', dlgid=dlgid,
                msg='ERROR: EXCEPTION {0}'.format(err_var))
            return
            ...
            if dlgid in allConfigs:
                for key in d:
                    ...
                    if key in allConfigs[dlgid]:
                        ...
                        sql += """   ( now(),now(),'{fvalue}',{medida_id},{ubicacion_id}),""".format(fvalue=fvalue,
                                                                                                     medida_id=medida_id,
                                                                                                     ubicacion_id=ubicacion_id)
                    else:
                        log(module=__name__, function='insert_spx_datos', level='ERROR', dlgid=dlgid,
                            msg='Not exist key {0} in config'.format(key))
            else:
            ...
            if dlgid in allConfigs:
                for key in d:
                    ...
                    if key in allConfigs[dlgid]:
                        ...
                        ubicacion_id = allConfigs[dlgid]['ubicacion_id']
                        sql += """
                                    ( now(),now(),'{fvalue}',{medida_id},{ubicacion_id}),""".format(fvalue=fvalue,
                                                                                                    medida_id=medida_id,
                                                                                                    ubicacion_id=ubicacion_id)
                    else:
                        log(module=__name__, function='insert_spx_datos', level='ERROR', dlgid=dlgid,
    ...
    def read_dlg_conf(self, dlgid):
        ...
        for row in results:
            ...
            d[(canal, pname)] = value
            log(module=__name__, function='read_dlg_conf', dlgid=dlgid, level='ERROR', msg='BD conf: [{0}][{1}]=[{2}]'.format(canal, pname, d[(canal, pname)]))
